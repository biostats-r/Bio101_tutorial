...
$x_1$ og $x_2$ osw er forskjellige lengder til fiskene.
$x_n$ betyr at det kan være $n$ antall fisker og størrelser.
...
I figuren har vi tegnet en standard avvik med den røde streken.
...
$$sd = \frac{\sqrt{(x_1 - \hat{y})^2 + (x_2 - \hat{y})^2 + (x_3 - \hat{y})^2 + \dots + (x_n - \hat{y})^2}}  {n - 1}$$

$x_1$ og $x_2$ osw er forskjellige tall fra en variable, for eksempel de forskjellige lengdene fra fiskene.
...
Standard feil av gjennomsnittet er en slags standard avvik i forskjellige verdier av prøvens gjennomsnitt. 
Kort sagt er standardfeilen av gjennomsnittet i en statistikk er ikke noe annet enn standardavviket i samplingsfordelingen.

Formelen for standard feil av gjennomsnittet er:
$se = sd / sqrt(n)$
...
Men hvis vi vil si noe om usikkerheten rundt en estimert gjennomsnitt, så bruker vi standard feil av gjennomsnittet.
...
Hvis vi skal beregne standard avvik av bakterier for kimtall datasettet kan vi bruke `sd()` funksjonen. 
Vi definerer først datasett, altså **kimtall** og deretter for hvilken variabel vi vil beregne standard avvik.
...
For å beregne **standard feil av gjennomsnittet** trenger vi flere ting:
...
La oss nå sette alt sammen og beregne gjennomsnitt og standard feil av gjennomsnittet for kimtall i de tre forskjellige vann.
...
Tabell og figur tekst er ganske viktig.
[Her](https://biostats.w.uib.no/hvordan-skrive-figurtekst/){target="_blank"} er litt mer informasjon om dette, hvis du aldri har skrevet en tekst til en tabell eller figur.
